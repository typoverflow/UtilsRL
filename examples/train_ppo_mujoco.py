...
from UtilsRL.rl.normalizer import RunningNormalizer, StaticNormalizer
...
args["obs_shape"] = env.observation_space[0]
args["action_shape"] = env.action_space[0]
...
actor = SquashedGaussianActor(
    ...
    device=args.device, reparameterize=False, conditioned_std=True
).to(args.device)
...
buffer = TransitionReplayPool(args.obs_space, args.action_space, args.buffer_size, entra_fields={
...
env = gym.make(task)
for i_epoch in Monitor("PPO Training").listen(args.max_epoch):
    buffer.clear()
    obs, done = env.reset(), False
    sample_ph = buffer.get_placeholder(args.max_traj_length)
    traj_length = traj_return = 0
    for env_step in range(args.sample_per_epoch):
        ...
        sample_ph["obs"][traj_length-1] = obs
        sample_ph["action"][traj_length-1] = action
        sample_ph["logprob"] = logprob
        sample_ph["next_obs"][traj_length-1] = next_obs
        sample_ph["reward"][traj_length-1] = reward / args.reward_scale
        sample_ph["done"][traj_length-1] = done
        sample_ph["value"][traj_length-1] = value
        ...
        if done or timeout or epoch_ended:
            if not done:
                ...
                last_v = 0
            gae, ret = compute_gae(sample_ph["reward"][:traj_length], sample_ph["value"][:traj_length], last_v=last_v)
            sample_ph["return"][:traj_length] = ret
            sample_ph["advantage"][:traj_length] = gae
